const express = require('express');
const { OpenAI } = require('openai');
const axios = require('axios');
const fs = require('fs');
const path = require('path');
const { v4: uuidv4 } = require('uuid');

const router = express.Router();
const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

const ELEVENLABS_API_KEY = process.env.ELEVENLABS_API_KEY;
const ELEVENLABS_VOICE_ID = process.env.ELEVENLABS_VOICE_ID;
const AUDIO_FOLDER = path.join(__dirname, '..', 'public', 'audio');

// Ensure audio folder exists
if (!fs.existsSync(AUDIO_FOLDER)) {
  fs.mkdirSync(AUDIO_FOLDER, { recursive: true });
}

router.post('/realtime', async (req, res) => {
  const userMessage = req.body.message;
  if (!userMessage) {
    return res.status(400).json({ error: 'Message is required' });
  }

  try {
    const stream = await openai.chat.completions.create({
      model: 'gpt-3.5-turbo',
      messages: [
        { role: 'system', content: 'You are a helpful AI voice assistant.' },
        { role: 'user', content: userMessage }
      ],
      stream: true
    });

    let buffer = '';
    const audioChunks = [];

    for await (const chunk of stream) {
      const delta = chunk.choices?.[0]?.delta?.content;
      if (!delta) continue;

      buffer += delta;

      const shouldFlush = buffer.endsWith('.') || buffer.split(' ').length > 5;
      if (shouldFlush) {
        const filename = `${uuidv4()}.mp3`;
        const filepath = path.join(AUDIO_FOLDER, filename);
        const audioUrl = `${process.env.PUBLIC_URL}/audio/${filename}`;

        const audioResponse = await axios({
          method: 'post',
          url: `https://api.elevenlabs.io/v1/text-to-speech/${ELEVENLABS_VOICE_ID}`,
          headers: {
            'xi-api-key': ELEVENLABS_API_KEY,
            'Content-Type': 'application/json',
            'Accept': 'audio/mpeg'
          },
          data: {
            text: buffer,
            model_id: 'eleven_monolingual_v1',
            voice_settings: { stability: 0.4, similarity_boost: 0.75 }
          },
          responseType: 'arraybuffer'
        });

        fs.writeFileSync(filepath, audioResponse.data);
        audioChunks.push(audioUrl);
        buffer = '';
      }
    }

    // Final buffer flush
    if (buffer.trim()) {
      const filename = `${uuidv4()}.mp3`;
      const filepath = path.join(AUDIO_FOLDER, filename);
      const audioUrl = `${process.env.PUBLIC_URL}/audio/${filename}`;

      const audioResponse = await axios({
        method: 'post',
        url: `https://api.elevenlabs.io/v1/text-to-speech/${ELEVENLABS_VOICE_ID}`,
        headers: {
          'xi-api-key': ELEVENLABS_API_KEY,
          'Content-Type': 'application/json',
          'Accept': 'audio/mpeg'
        },
        data: {
          text: buffer,
          model_id: 'eleven_monolingual_v1',
          voice_settings: { stability: 0.4, similarity_boost: 0.75 }
        },
        responseType: 'arraybuffer'
      });

      fs.writeFileSync(filepath, audioResponse.data);
      audioChunks.push(audioUrl);
    }

    res.status(200).json({ audioChunks });
  } catch (err) {
    console.error('❌ Error in /realtime:', err.message);
    res.status(500).json({ error: 'Failed to generate response or audio' });
  }
});

module.exports = router;

