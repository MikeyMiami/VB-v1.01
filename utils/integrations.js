    ...
    db.get(`SELECT * FROM Integrations WHERE id = ?`, [integrationId], async (err, integration) => {
      ...
      try {
        switch (integration.integration_type) {
          case 'hubspot':
            console.log(`Fetching leads from HubSpot${listId ? ` list ID ${listId}` : ''}`);
            ...
            if (listId) {
              ...
              const listResponse = await membershipsApi.getPage(listId, 100, undefined, ['phone'], false, false); // Limit 100 for now; add pagination if needed
          ...
          case 'google_sheets':
            ...
            const doc = new GoogleSpreadsheet(creds.sheet_id || reject(new Error('sheet_id missing in creds')));
        ...
        console.error(`API error in fetchLeads for type ${integration.integration_type}:`, apiErr.message);
        reject(apiErr);
      }
    });
  });
      ...
      try {
        switch (integration.integration_type) {
          case 'google_calendar':
            ...
            break;
          case 'calendly':
            console.log('Booking appointment in Calendly');
        ...
        console.error(`API error in bookAppointment for type ${integration.integration_type}:`, apiErr.message);
        reject(apiErr);
      }
    });
  });
